...
DATA$disabled <- as.integer(DATA$disabled)
...
function(){
  ...
  SPECS <- unique(Fxn_SPECS(TechVarlist=c("disabled","disabled_self","disabled_spouse","disabled_child","disabled_close","disabled_member"),mainD = mainD, mainF=mainF))

  SPECS <- rbind(
    data.frame(SPECS[ (SPECS$f %in% mainF & SPECS$d %in% mainD & SPECS$TechVar %in% "disabled" & SPECS$level %in% "Pooled"),], nnm="fullset"),
    data.frame(SPECS[ (SPECS$f %in% mainF & SPECS$d %in% mainD & SPECS$TechVar %in% "disabled" & SPECS$level %in% "Pooled"),], nnm="optimal"),
    data.frame(SPECS[!(SPECS$f %in% mainF & SPECS$d %in% mainD & SPECS$TechVar %in% "disabled" & SPECS$level %in% "Pooled"),], nnm="optimal"))
  ...
  function(fit){
    ...
    if(!paste0(disasg,"_",level,"_",TechVar,"_",names(FXNFORMS)[f],"_",names(DISTFORMS)[d],"_",nnm,".rds") %in% 
      ...
      res <- lapply(
        ...
        intercept_shiftersM = list(Svarlist=ArealistX,Fvarlist=c("Survey","Ecozon")),
        drawlist = drawlist,
        wvar = "Weight",
        yvar = "HrvstKg",
      ...
      if(!(TechVar %in% "disabled" & nnm %in% "optimal" & level %in% "Pooled" & disasg %in% "CropID" & f %in% 2 & d %in% 1)){
        res$rk_dist <- NULL
        res$rk_mean <- NULL
        res$rk_samp <- NULL
        res$el_samp <- NULL
