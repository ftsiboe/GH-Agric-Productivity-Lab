  ...
  function(fit){
    ...
    if(!paste0(disasg,"_",level,"_",TechVar,"_",names(FXNFORMS)[f],"_",names(DISTFORMS)[d],"_",nnm,".rds") %in% list.files("results/estimations/")){
      ...
      res <- lapply(
        ...
        ulist = list(Svarlist=c("lnAgeYr","lnYerEdu","CrpMix"),Fvarlist=c("Female","Survey","Ecozon","Credit","EqipMech","OwnLnd")),
        ulistM= list(Svarlist=c("lnAgeYr","lnYerEdu","CrpMix"),Fvarlist=c("Female","Survey","Ecozon","Credit","EqipMech","OwnLnd")),
      ...
      if(!(TechVar %in% "ext_available" & nnm %in% "optimal" & level %in% "Pooled" & disasg %in% "CropID" & f %in% 2 & d %in% 1)){
        res$rk_dist <- NULL
        res$rk_mean <- NULL
        res$rk_samp <- NULL
        res$el_samp <- NULL
