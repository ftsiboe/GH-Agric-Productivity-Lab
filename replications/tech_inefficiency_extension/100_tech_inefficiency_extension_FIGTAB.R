...
wb <- openxlsx::loadWorkbook("results/tech_inefficiency_extension_results.xlsx")
openxlsx::writeData(wb, sheet = "msf",res , colNames = T, startCol = "A", startRow = 1)
openxlsx::saveWorkbook(wb,"results/tech_inefficiency_extension_results.xlsx",overwrite = T)

# Fig - Heterogeneity          
rm(list= ls()[!(ls() %in% c(Keep.List))])
res <- readRDS("results/estimations/CropID_Pooled_ext_available_TL_hnormal_optimal.rds")$disagscors
...
fig <- fig_heterogeneity00(res=res,y_title="Level difference (No extension minus some extension)\n")
...
# Fig - Robustness              
rm(list= ls()[!(ls() %in% c(Keep.List))])
fig_robustness(y_title="\nPercentage point difference  [No extension minus some extension]",
               res_list = c("results/estimations/CropID_Pooled_ext_available_CD_hnormal_optimal.rds",
                            list.files("results/estimations/",pattern = "CropID_Pooled_ext_available_TL_",full.names = T)))

# Fig - Matching TE      
rm(list= ls()[!(ls() %in% c(Keep.List))])
fig_input_te(y_title="\nGap associated with extension (%)",tech_lable=c("Full sample", "No extension sample", "some extension sample"))
...
# Fig - TREND 
ef_mean <- readRDS("results/estimations//CropID_Pooled_ext_available_TL_hnormal_optimal.rds")$ef_mean
